...
function refreshData(data, prevData, vars) {
    ...
    data.runes.forEach(rune => {
        if (prevData.uniqueToId.gridRunes[rune.rune_id]) {
    ...
    data.unit_list.forEach(monster => {
        ...
        if (!filtered) {
            if (prevData.uniqueToId.gridMons[monster.unit_id]) {
    ...
    data.rune_craft_item_list.forEach(craft => {
        if (prevData.uniqueToId.gridCrafts[craft.craft_item_id]) {
...
function mapCraft(craft, craft_id) {
    ...
    map.type = (craft['craft_type'] == 1) ? 'E' : 'G';
